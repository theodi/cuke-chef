@staging-mysql-certificate-theodi-org-cucumber
Feature: Build a functioning MySQL server
  ...
  Background:
    * I have a server called "staging-mysql-certificate-theodi-org-cucumber"
    * "staging-mysql-certificate-theodi-org-cucumber" is running "ubuntu" "precise"
    * "staging-mysql-certificate-theodi-org-cucumber" should be persistent
    * "staging-mysql-certificate-theodi-org-cucumber" has an IP address of "192.168.87.51"

    * the following environment has been uploaded:
      | environment | environment_path |
      | cucumber-staging.rb | ./environments/  |
    ...
    * "staging-mysql-certificate-theodi-org-cucumber" is in the "cucumber-staging" environment
    * "staging-mysql-certificate-theodi-org-cucumber" has been provisioned

    * all of the cookbooks in "./cookbooks" have been uploaded
    * all of the cookbooks in "./site-cookbooks" have been uploaded

    * the "open-data-certificate-attrs" role has been added to the "staging-mysql-certificate-theodi-org-cucumber" run list
    * the "mysql-node" role has been added to the "staging-mysql-certificate-theodi-org-cucumber" run list
    * the chef-client has been run on "staging-mysql-certificate-theodi-org-cucumber"

    * I ssh to "staging-mysql-certificate-theodi-org-cucumber" with the following credentials:
      | username | keyfile |
      | $lxc$    | $lxc$   |

  Scenario: Can connect to the provisioned server via SSH authentication
    When I run "hostname"
    Then I should see "staging-mysql-certificate-theodi-org-cucumber" in the output
